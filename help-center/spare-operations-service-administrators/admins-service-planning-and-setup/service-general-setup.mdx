...
**Home Screen Trip Mode**: Configure which booking options are displayed on the rider app home screen. This setting controls the primary trip booking interface that riders see when they open the app:

<Info>
The Home Screen Trip Mode setting allows you to customize the rider booking experience based on your service's operational model and rider needs.
</Info>

- **Scheduled and Leave Now**: Shows both scheduled booking options and immediate "leave now" booking. This is the most flexible option for riders who may need both advance planning and immediate transportation.
- **Scheduled Only**: Displays only scheduled booking options, requiring riders to book trips in advance. This is ideal for services that operate primarily on scheduled routes or need advance planning for resource allocation.
- **Leave Now Only**: Shows only immediate booking options for on-demand service. This setting is perfect for services that focus on immediate response and don't require advance scheduling.

<Tip>
Consider your service model when selecting this setting. Paratransit services often benefit from "Scheduled Only" mode, while urban on-demand services might prefer "Leave Now Only" or the combined "Scheduled and Leave Now" option.
</Tip>
...
<Warning>
Changes to the Home Screen Trip Mode setting affect the rider app interface immediately. Ensure you communicate any changes to your riders if you modify this setting for an active service.
</Warning>
...
- **Home Screen Trip Mode**: Immediately (affects rider app interface)
