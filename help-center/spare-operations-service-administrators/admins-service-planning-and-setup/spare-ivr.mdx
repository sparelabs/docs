---
title: "Spare IVR"
description: "Interactive Voice Response (IVR) features that enable riders to book, cancel, and check ride status over the phone."
---
...
Spare IVR is a suite of IVR features that enable riders to book, cancel, and check the status of rides over the phone with an IVR agent.

## What is the value?

Spare IVR is valuable for Agencies and Riders:
...
Spare IVR automatically identifies and authenticates the rider by recognising the rider's phone number. If Spare IVR doesn't recognize the rider's phone number the rider is forwarded straight to a call center agent 
...
To set up the Spare IVR feature, your organization only needs to provide a forwarding number that goes to your customer service/reservations team. Spare will use this number as a backup in case the customer/caller wants to bypass the IVR. 
...
All Spare IVR features only work in English

We use the incoming phone number to authenticate riders, so all Spare IVR features only work if the rider is calling from the phone number linked to their account.
