...
description: "Step-by-step framework for investigating Engine decisions and understanding trip logic"
---
...
However, there are times when you really need to know why something happened. So how do you investigate this? In this article, we'll walk you through the **investigation framework** to get a thorough understanding of what was the logic behind the Engine's decision.

**See also:**

- [Driver Activity](/help-center/spare-operations-dispatchers/dispatchers-advanced/investigating-a-drivers-activity)
...
1. Locate the request via the rider's profile. For that, type in the rider's name in the main search bar, open their profile and scroll down to view the requests history
2. Open the request you'd like to investigate. Scroll down to view the **status changes** and the **activity log**

From the Request Information Page, locate and write down the following details:

- **The date & time** the trip was supposed to happen (look for the "Intent time" and "Request pickup time")
...
Take a look at the table below to learn what each status means:
