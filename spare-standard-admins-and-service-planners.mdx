...
**Why Enable:** This is useful for integrations where the external ID is managed by an external system and should not be editable in Spare. It ensures data integrity by preventing manual edits to IDs that are synchronized from external systems.

**Pre-requisites:**
- None.

**Notes:**
- This is a UI change that prevents editing of the External Rider ID field in the admin panel.
