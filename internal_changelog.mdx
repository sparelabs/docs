<Update label="Thursday, October 23, 2025" description="6 Updates" tags={["Operations", "Admin", "Resolve"]}>

## 🎯 Operations
### 📊 New Revenue-Hour Productivity Metrics

**What's New**  
You can now measure operational efficiency using revenue-hour-based metrics, such as Requests per Revenue Hour (RPRH) and Boardings per Revenue Hour (BPRH). These new metrics are available in the Live Requests dashboard and can be set as the primary productivity metric for your organization.
...
#### Where You'll See It
- The new metrics are visible in the summary panel on the *Rides > Live Requests* page.
- Administrators can configure the primary productivity metric in *System Settings > General > Operations* (in the "Productivity" section).

#### What You Can Do
- Track productivity based on "Revenue Hours," a new concept defined as the time a vehicle is actively performing service (from the first completed pickup to the last drop-off, excluding breaks).
- Choose between Boardings/Vehicle Hour, Requests/Vehicle Hour, or the new Requests/Revenue Hour as your organization's primary productivity metric.

#### Why This Matters
Metrics based on revenue hours provide a more accurate measure of efficiency by excluding non-service time like paid breaks or travel to the first pickup. This gives dispatchers and managers a clearer view of performance during active service periods.
</Accordion>

---

## ⚙️ Admin
### 🔐 New OAuth Scope for Accessing Sensitive Data

**What's New**  
A new `sensitiveInformation:read` OAuth scope has been added, allowing administrators to grant specific API applications permission to view unmasked sensitive data. This is useful for trusted integrations that need to see Personally Identifiable Information (PII).

**Pre-requisites**
- Requires Administrator access
- This scope is only effective if data masking is enabled for your organization.

<Accordion title="See More">

#### Where You'll See It
- *System Settings > Apps > (select an app or 'Create App') > Scopes* dropdown menu.

#### What You Can Do
- When creating or editing an API application, you can now select the `sensitiveInformation:read` scope from the list.
- An application using a token with this scope will be able to retrieve sensitive fields (like full names or phone numbers) in their entirety, bypassing any data masking rules.

#### Why This Matters
This provides granular control for trusted integrations and internal tools that require access to complete PII. It avoids the need to disable data masking for the entire organization or for all API users, enhancing security.
</Accordion>

---
### 🗺️ Faster Loading for Geographies Page

**What's New**  
The Geographies page now loads significantly faster, especially for organizations with many complex service areas. We've optimized the way geography data is fetched, reducing load times and improving the overall responsiveness of the page.
...
#### What You Can Do
- Experience quicker access and interaction when viewing, creating, or editing your organization's geographies. The map and list of geographies will now populate much more rapidly.

#### Why This Matters
Previously, organizations with a large number of service zones could experience significant delays when loading the Geographies page. This update streamlines data retrieval by fetching all information in a single, efficient request, minimizing wait times and improving the workflow for administrators.
</Accordion>
...
**What's New**  
The Spare platform now officially supports a European (EU) hosting region. This allows organizations to have their services and data hosted locally within the EU, ensuring compliance with regional data residency requirements like GDPR.

**Pre-requisites**
- This is a platform-level setting configured by Spare during organization setup.

<Accordion title="See More">

#### Where You'll See It
- This is a foundational, non-UI change. For organizations hosted in the EU, all API interactions will be directed to the new EU endpoints (e.g., `https://api.eu.sparelabs.com`).

#### What You Can Do
- If your organization requires data to be stored within the European Union, you can now request to be hosted in our EU region.

#### Why This Matters
This update enables services to meet GDPR and other European data sovereignty rules, providing a compliant and secure solution for partners operating in the region.
</Accordion>

---
### 🐛 Bug Fixes

- **App Authorization Now Uses Correct Regional API**
  - _Resolution_: Fixed a bug where the app authorization page for organizations in non-US regions (like Canada) would fail because it was incorrectly trying to fetch app details from the US API. The authorization process now correctly uses the API host for the organization's specific region.
...
- **Appointment Date No Longer Jumps During Editing (ENG-854)**
  - _Resolution_: Fixed an issue in the Engage module where editing an existing appointment would cause the date and time to automatically jump to the next available slot. The system now preserves the original appointment's date and time when the editing modal is opened.
