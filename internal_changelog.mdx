...
**What's New**  
You can now create and manage custom categories for Work Orders. These categories can be assigned to work orders to improve organization and can be used to filter the main Work Orders list.
...
#### Where You'll See It
- *EAM > Settings > Work Order Categories*: A new page to create, view, edit, and delete categories.
- *EAM > Work Orders > New/Edit Work Order*: A new "Category" dropdown field for assigning a category.
- *EAM > Work Orders* (list view): A new "Category" column and a "Category" filter option have been added.

#### What You Can Do
- Create custom categories such as "Preventive Maintenance," "Unscheduled Repair," or "Safety Inspection."
- Assign a category when creating or editing a work order.
- Filter the work order list to see only orders of a specific category.

#### Why This Matters
Previously, all work orders were in a single, undifferentiated list, making it hard to track different types of maintenance. Now, you can segment work orders by category, which simplifies filtering, reporting, and managing different maintenance workflows.
</Accordion>
...
- **Service Discovery for Services with Custom Time Rules (BUS-3786)**
  * _Resolution_: Fixed an issue where services with specific operating-hour rules might not appear in the service selection dropdown when booking a trip. The system now correctly displays these services during initial discovery and validates the time only after a time has been chosen.
...
- **Clearing Province/State in Rider Addresses (ENG-835)**
  * _Resolution_: Fixed an issue in rider profiles where the "Country" and "Province/State" fields in an address could not be cleared. The "None" option in the dropdown is now selectable, allowing these fields to be emptied when editing an address.
