<Update label="Thursday, October 23, 2025" description="7 Updates" tags={["Operations", "Analytics", "Admin", "Resolve"]}>

## 🎯 Operations
### 📊 New Productivity Metrics for Live Operations

**What's New**  
We've introduced new productivity metrics, including Requests per Revenue Hour (RPRH), to provide deeper insights into your operational efficiency. These metrics are now available for calculation and display on the *Live Requests* page, with the system automatically accounting for duty status and break times.

**Pre-requisites**
- Requires Administrator access to configure
- The most recent calculation method may require the `useV2RevenueHours` feature flag

<Accordion title="See More">

#### Where You'll See It
- *System Settings > General* (new **Productivity Metric** dropdown)
- *Rides > Live Requests* (new data points in the summary view)

#### What You Can Do
- As an administrator, you can now select your organization's primary productivity metric (e.g., Boardings/Vehicle Hour, Requests/Vehicle Hour, or Requests/Revenue Hour).
- The system will calculate this metric in real-time for in-progress duties, subtracting any break times to ensure accuracy. For completed duties, it uses precomputed values.

#### Why This Matters
Previously, tracking metrics like RPRH required manual, after-the-fact calculation. This update automates the process, providing live, accurate efficiency data directly within the platform and helping you make faster, more informed operational decisions.
</Accordion>

---

## 📊 Analytics
### 📋 Analytics Report & Dashboard Templates

**What's New**  
You can now create, manage, and clone analytics reports and dashboards from templates. This simplifies the process of creating standardized reports and ensures consistency across your organization.

**Pre-requisites**
- Requires Administrator access
- Access to analytics features may depend on the `AnalyticsV2` feature flag

<Accordion title="See More">

#### Where You'll See It
- *Analytics > Templates* (a new section for managing report and dashboard templates)
- A new **Clone from Template** option will be available when creating new reports or dashboards.

#### What You Can Do
- Build a base report or dashboard with specific queries and visualizations, and save it as a template.
- Allow other users to quickly create new, pre-configured analytics assets by cloning an existing template.

#### Why This Matters
Building complex reports from scratch can be time-consuming and lead to inconsistencies. Templates allow you to establish a "gold standard" for reporting, saving time and ensuring everyone is analyzing data using the same queries and formats.
</Accordion>

---

## ⚙️ Admin
### 🗺️ Faster Geography Page Loading

**What's New**  
We've optimized how geographic data is loaded, resulting in a much faster and more responsive experience on the Geographies page. This improvement is especially noticeable for organizations managing a large number of complex zones.
...
#### Where You'll See It
- *System Settings > Geographies*

#### What You Can Do
- Navigate, create, and edit geographies without the long load times previously experienced, particularly in environments with hundreds of zones. The page now fetches and displays data in more efficient batches.

#### Why This Matters
Waiting for a large number of complex geographies to load on the map was slow and disrupted administrative workflows. This performance improvement significantly reduces wait times, allowing administrators to manage service areas more quickly and efficiently.
</Accordion>

---
### 🇪🇺 Platform-Wide Support for EU Region

**What's New**  
The Spare platform now officially supports the European Union (EU) as a distinct operational region. This update ensures data residency and compliance for our European partners, with services and APIs hosted on dedicated EU infrastructure.

**Pre-requisites**
- Requires Super admin permissions for initial setup and configuration

<Accordion title="See More">

#### Where You'll See It
- This is a platform-wide infrastructure update. EU-based organizations will use regional API URLs (e.g., `api.eu.sparelabs.com`).
- Services including AI features, data pipelines, and other regionalized tools will automatically use EU-based resources for relevant organizations.

#### What You Can Do
- Operate your service entirely within the EU region, ensuring all organizational and rider data is processed and stored locally.

#### Why This Matters
For organizations operating in the EU, data residency is a critical legal and operational requirement. This update provides the necessary infrastructure to meet GDPR and other regional data-handling regulations, enabling full platform use while maintaining compliance.
</Accordion>

---
### 🔐 New OAuth Scope for Viewing Sensitive Data

**What's New**  
We've added a new, granular OAuth scope called `sensitiveInformation:read`. This allows you to grant specific API applications the ability to view sensitive information (like rider names or contact details) without masking.

**Pre-requisites**
- Requires Administrator access
- This is relevant for organizations that use data masking features.

<Accordion title="See More">

#### Where You'll See It
- *System Settings > Apps > (Create or Edit an App)*, within the **Scopes** dropdown menu.

#### What You Can Do
- When creating or editing an OAuth application, you can now assign the `sensitiveInformation:read` scope.
- API tokens generated by this app will be able to retrieve unmasked data, while tokens from other apps without this scope will continue to see masked data as per your organization's settings.

#### Why This Matters
Previously, granting API access often meant choosing between all-or-nothing access to sensitive data. This new scope provides precise control, allowing you to enable data access for trusted internal tools or integrations while maintaining strict data masking for all other API clients.
</Accordion>

---
### 🐛 Bug Fixes

- **App Authorization Page Correctly Uses Regional API**
  - _Resolution_: Fixed an issue where the app authorization page (`/:orgId/app/authorize`) for organizations in non-US regions (e.g., Canada) would fail to load. The page now correctly uses the organization's specific regional API endpoint instead of defaulting to the US.
...
- **Appointment Date No Longer Jumps During Edits (ENG-854)**
  - _Resolution_: Fixed an issue in the *Engage > Appointments* booking form where editing an existing appointment would cause the date and time to unexpectedly change to the next available slot. The form now correctly retains the original appointment's date and time upon opening the edit modal.
